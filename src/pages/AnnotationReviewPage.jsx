...
import {
  fetchAnnotationsReview,
  fetchAnnotationsSimulation,
} from "../TankStackQuery/ApiCalls";

function AnnotationReviewPage() {
  ...
  //   setClassList,
  // });
  const { data: FetchData, isPending,isFetching } = useQuery({
    queryKey: ["fetchAnnotationsSimulation"],
    queryFn: async () => {
      const resp = await fetchAnnotationsReview({
        user_id,
        project_name,
        model_id: ai_model_name,
        inference_url_dic: upload_data,
      });
      ...
      <div className="row-span-1 w-full px-10">
        <div className=" w-full grid grid-cols-12 px-10 min-h-20 h-10/12 border-b border-b-[#8E8E8E]">
          ...
          <div className="col-span-5 h-full grid grid-cols-9">
            <div className="col-start-3 col-span-3 grid-cols-7 flex justify-center items-center gap-4">
              <img
            ...
            <div className="col-span-4 p-2">
              <button
                className="w-full h-full rounded-full bg-GlobalPrimary text-white text-2xl cursor-pointer"
                onClick={() => {
                  navigate("/report", {
                    state: {
                      images: images,
